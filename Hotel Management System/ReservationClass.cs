...
namespace Hotel_Management_System
{
    ...
    class ReservationClass
    {
        ...
        {
            string selectQuery = "SELECT * FROM `rooms` WHERE `room_type`=@type AND `status` ='Kosong'";
        ...
        {
            string insertQuerry = "INSERT INTO `reservation`(`client_id`, `reserv_id`, `date_in`, `date_out`) VALUES (@Gid,@Rno,@Din,@Dout)";
        ...
        {
            string insertQuerry = "UPDATE `reservation` SET `client_id`=@Gid,`room_no`=@Rno,`date_id`=@Din,`date_out`=@Dout WHERE `RecervId`=@rid";
            MySqlCommand command = new MySqlCommand(insertQuerry, connect.GetConnection());
